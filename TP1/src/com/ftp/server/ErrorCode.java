...
public final class ErrorCode {
	...
	public static String getMessage(String _ErrorCode, String _paramCode) {
		...
		try {
			...
			_paramCode="";
			System.err.println("ErrorCode: interesting, unknown error: "+ _ErrorCode +"\n");			
			e.printStackTrace();
		}
		finally{
			if (!(_paramCode == null)) {
	...
	private static void populateCode() {
		ftpErrorMap
				.put("100","The requested action is being initiated, expect another reply before proceeding with a new command..");
		...
		ftpErrorMap.put("202", "Command not implemented, superfluous at this site.");
		...
		ftpErrorMap
				.put("220"," 	Service ready for new user on PARAM. login using the USER command.");
		...
		ftpErrorMap.put("226", "Closing data connection. Requested file action successful.PARAM");
		...
		ftpErrorMap
				.put("232","Logout command noted, will complete when transfer done.");
		...
		ftpErrorMap.put("350", "Requested file action pending further information.PARAM");
		ftpErrorMap
				.put("400","The command was not accepted and the requested action did not take place, but the error condition is temporary and the action may be requested again.");
		ftpErrorMap.put("421", "Service not available, closing control connection. This may be a reply to any command if the service knows it must shut down.PARAM");
		ftpErrorMap
		...
		ftpErrorMap.put("451", "Requested action aborted. Local error in processing.");
		ftpErrorMap.put("452", "Requested action not taken. Insufficient storage space in system.File unavailable (e.g., file busy).");
		ftpErrorMap
				.put("500","Syntax error, command unrecognized and the requested action did not take place. This may include errors such as command line too long.");
		...
		ftpErrorMap.put("552", "Requested file action aborted. Exceeded storage allocation (for current directory or dataset).");
		ftpErrorMap.put("553","Requested action not taken. File name not allowed.");
		ftpErrorMap.put("600", "Replies regarding confidentiality and integrity");
